...
## 📋 最近の更新（Changelog v2.4）

| ID | 機能 | 説明 |
|-----|------|------|
| **B1** | 謎の銘柄除去（DOM走査修正） | MoneyForward スクレイパーの DOM 走査ロジックを改善し、スクラップ時に不要な銘柄が取得される問題を解決 |
| **B6** | 金融機関名表示 | 保有資産・クレカページで金融機関（銀行・カード会社）の名称を表示 |
| **C1** | クレカ 3 枚取得対応 | 複数クレジットカード（最大 3 枚）のデータを同時に取得・表示可能 |
| **C2** | 現在価格表示改善 | 資産一覧に currentPriceJpy 列を追加し、リアルタイムな現在値を表示 |
| **C3** | UI/UX 改善 | 資産一覧・クレカページの列構成を見直し、より直感的なレイアウトに統一 |
| **T01** | 資産一覧 currentPriceJpy ソート + USD 外貨表示 | 「現在値」カラムでソート可能。米国株は USD $xxx.xx 形式で表示 |
| **T02** | スクレイパー CATEGORY_MAP 拡充 + FUND/PENSION 動的パーサ修正 | MF の DOM 変化に強い動的パーサーを実装 |
| **T03** | 引き落とし bank_account リセットバグ修正 + cc_account_mapping 自動リマップ | スクレイプ後も手動設定値が保持されるよう修正 |
| **T03** | 3段階残高警告（urgent/danger/caution）追加 | 引き落とし予定と残高を比較して警告バナー・バッジ表示 |
| **T04** | 配当月別計算を months フィールド活用に改善 + per_payment_jpy 追加 | 月別配当分布の精度向上 |
| **T05** | 設定リセットバグ3件修正（空文字列保存・NaN・PM2 API_KEY）+ 最終更新日時表示 | 設定保存の安定性改善 |
| **T06** | /logs ページ新規作成 | スクレイプ/API/MCP/Discord のログを4タブで閲覧可能 |
| **T07** | MCP ツール3件追加 | get_withdrawal_summary, get_financial_summary, get_investment_advice_context |
| **T08** | /income-expense 廃止・MonthlyExpenseChart をダッシュボードに統合 | ページ削減でUX改善（/ にリダイレクト） |
| **T09** | daily_totals 前月比・前年比・カテゴリ別前日比カラム追加 | ダッシュボードに前月比・前年比・カテゴリ別差分を表示 |
| **T10** | E2E テスト Strict mode violation 修正 | getByRole が aria-label にも match する問題を nav スコープで解決。69 passed, 2 skipped, 0 failed |
| **T11** | PM2/マイグレーション再発防止手順追加 | docs/operations/deployment.md 新規作成・scripts/reload.sh コメント追加 |
## 🛠 技術スタック
...
# マイグレーション + PM2 サービス再起動を一括実行（2回目以降推奨）
bash scripts/reload.sh
```

初回時のみ実行。`data/assetbridge_v2.db` が作成されます（bun:sqlite WAL モード）。
新しいマイグレーションが追加された場合は `bash scripts/reload.sh` で反映してください。
...
| ページ | 機能 |
|--------|------|
| **ダッシュボード** | 総資産・推移グラフ・資産構成・AI分析コメント・前月比・前年比・カテゴリ別差分・月別支出予定グラフ |
| **保有資産** | 株式・投信・現金等の詳細一覧（金融機関名・銘柄・数量・現在値・評価額・損益率） |
| **引き落とし管理** (`/withdrawals`) | クレカ・固定費の月次引き落とし管理、3段階残高警告（urgent/danger/caution） |
| **市況** | ニュース・リスク分析 |
| **配当** | 配当カレンダー・月別推定額 |
| **シミュレータ** | Monteカルロシミュレーション（リスク・リターン評価） |
| **ログ** (`/logs`) | スクレイプ/API/MCP/Discord ログ閲覧（4タブ） |
...
**引き落とし・サマリー**
- `get_withdrawal_summary` — 月次引き落とし予定サマリー（クレカ+固定費）
- `get_financial_summary` — 資産・引き落とし・配当の総合サマリー
- `get_investment_advice_context` — AI 投資アドバイス用コンテキスト取得
