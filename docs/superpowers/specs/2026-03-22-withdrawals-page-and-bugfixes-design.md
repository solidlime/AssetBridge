...
- `quarterly`: `withdrawal_month` は四半期の**開始月**（例: 1 → 1/4/7/10月に引き落とし）。計算ロジック: `開始月 + 3n ヶ月（n=0,1,2,3）`。月別集計では該当4ヶ月にのみ金額を計上（他月は0）。`getMonthlyWithdrawalSummary` は対象月が引き落とし月かどうかを `(month - withdrawal_month) % 3 === 0` で判定する。
...
**クレカ詳細 selector フォールバック方針:**
- CSS selector 失敗時は `logger.warn()` でログ出力し、該当カードの詳細フィールドを NULL で保存してスクレイプを継続（クラッシュしない）

**`scheduled_amount_jpy` 更新ポリシー:**
- 毎回の同期スクレイプ（設定ページ「今すぐ同期」または定期スケジュール）で最新値に上書き更新（`card_name` UNIQUE upsert）

**`getCreditCardDetails` レスポンス型:**
```typescript
const CreditCardDetailSchema = z.object({
  cardName: z.string(),
  cardType: z.string().nullable(),
  cardNumberLast4: z.string().nullable(),
  totalDebtJpy: z.number().nullable(),
  scheduledAmountJpy: z.number().nullable(),
  scrapedAt: z.string(),
});
// エンドポイント返却型: CreditCardDetailSchema[]
```

**固定費削除後の集計更新:**
- `deleteFixedExpense` Mutation 成功後、フロント側で React Query の `getFixedExpenses` と `getMonthlyWithdrawalSummary` を `invalidateQueries` でリアルタイム無効化・再取得する

**変更対象:** `apps/web/src/app/page.tsx` 「クレジットカード引き落とし予定」セクション
...
- **重複排除方法**: クレカ・固定費それぞれから `asset_id` を収集し、`DISTINCT asset_id` のセットを作成。各 `asset_id` の最新残高を1回だけ加算する（同一口座に複数のクレカ/固定費が紐づいていても残高は1回のみカウント）
- 残高残り = `SUM(残高 WHERE asset_id IN distinct_linked_asset_ids)` − クレカ引き落とし合計 − 固定費月次換算合計
...
- 配当月が確定した銘柄のみ月別集計を更新
- **不明フラグ実装方針（DB永続化）:**
  - スクレイプ時に配当データを `dividend_data` テーブル（新設）に永続化
  - `dividend_data` スキーマ: `(id PK, ticker TEXT UNIQUE, months TEXT, annual_jpy REAL, is_unknown INTEGER DEFAULT 0, scraped_at TEXT)`
  - minkabu・yfinance 両方で取得失敗時のみ `is_unknown = 1`
  - `dividendCache.ts` は DB から読み取りメモリキャッシュ（TTL: 24h）として機能。サーバ再起動でも DB から再ロードするためフラグは消えない
  - **API側**: 月別配当集計エンドポイントで `is_unknown = 1` の銘柄を除外（ゼロ埋めしない）
  - **フロント側**: 月別配当グラフに注釈「一部銘柄は配当月不明のため除外」を表示
...
| ファイル | 変更種別 | 内容 |
|---|---|---|
| `packages/db/src/schema/dividend_data.ts` | 新規 | dividend_data スキーマ（is_unknown含む） |
