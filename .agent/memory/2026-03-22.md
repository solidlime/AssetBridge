...
## ログシステム (2026-03実装)
- `packages/db/src/schema/app_logs.ts` — app_logs テーブル
- `packages/db/src/repos/app_logs.ts` — AppLogsRepo (insertLog/getLogs/cleanOldLogs)
- `apps/api/src/router/logs.ts` — getLogs query + clearLogs mutation
- `apps/api/src/index.ts` — POST ミューテーション系ミドルウェア（GET/ポーリングはスキップ）
- `apps/mcp/src/lib/logger.ts` — logMcp() fire-and-forget
- `apps/discord-bot/src/lib/logger.ts` — logDiscord() fire-and-forget
- `apps/web/src/app/logs/page.tsx` + `apps/web/src/components/LogViewer.tsx`

### db:migrate の注意事項
- `pnpm db:migrate` をルートから実行するとDBパスが解決できずエラーになる場合がある
- `cd packages/db && npx drizzle-kit migrate` で実行すること
- または `$env:ASSETBRIDGE_DB_PATH` を明示的に設定すること
...
### 修正
1. DB: POINT から同名 CASH レコードへ institution_name をコピー
2. スクレイパー恒久修正: CASH institution_name が空の場合、同名 POINT から自動継承

## 引き落とし管理ページ実装記録（2026-03-22）

### 新テーブル（3件）
- `fixed_expenses`: 固定費手動入力（name/amount_jpy/frequency/withdrawal_day/withdrawal_month/category/asset_id）
- `credit_card_details`: クレカ詳細スクレイプ結果（card_name UNIQUE upsert）
- `dividend_data`: minkabu 配当データキャッシュ（ticker UNIQUE upsert, is_unknown フラグ）

### 新 API エンドポイント（incomeExpense ルーター）
- `getFixedExpenses` / `addFixedExpense` / `updateFixedExpense` / `deleteFixedExpense`
- `getMonthlyWithdrawalSummary(month?)` — クレカ+固定費の月次合計
- `getCreditCardDetails` — スクレイプ済みクレカ詳細一覧

### スクレイパー修正（browser-scraper.mjs）
- 合計行除外: `table.querySelectorAll('tbody tr')` に変更 + クラスベースフィルタ
- CATEGORY_MAP 拡充（現金・預金, 株式（日本株）等を追加）
- `scrapeCreditCardDetails()` 追加 → card_name/type/last4/debt/scheduled 取得
- `scrapeMinkabuDividend()` 追加 → minkabu.jp から配当月・年間配当取得

### フロントエンド
- `/withdrawals` 新規ページ（月次サマリー・クレカ・固定費セクション）
- `FixedExpenseForm.tsx` 新規コンポーネント
- `/credit` → `/withdrawals` 301リダイレクト（next.config.ts）
- ナビゲーション「引き落とし管理」に変更
- ダッシュボード: 月次サマリー・固定費一覧を追加

### quarterly 計算ロジック
`((month - withdrawal_month) % 3 + 3) % 3 === 0` で引き落とし月判定

### dividendCache パターン
`apps/api/src/lib/dividendCache.ts` — DB読み取り+メモリキャッシュ(TTL:24h)。
`getDividendCalendar` で `is_unknown=true` 銘柄を除外（ゼロ埋めしない）。

### E2E テスト
- `tests/e2e/withdrawals.spec.ts` 追加（12テスト）
- 全体: 40 passed / 2 skipped

## バグ修正記録（2026-03-22 セッション4）

### cc_account_mapping の asset_id がスクレイプ後に無効になる問題（根本修正）
- 原因: CASH/POINT/FUND/PENSION 全削除→再 INSERT でオートインクリメント ID が変わり、
  `app_settings.cc_account_mapping`（card_name → asset_id）が stale になる
- 修正: `mf_sbi_bank.ts` に `cashNameToOldId` / `cashNameToNewId` マップ追加
  → 削除前後の ID 変化を追跡し、`cc_account_mapping` を自動リマップ

### bank_account がスクレイプ後にリセットされる問題（修正）
- 原因: DELETE → INSERT 時、MF スクレイプは `bankAccount=null` しか返さないため手動設定値が消える
- 修正: `mf_sbi_bank.ts` DELETE 前に `existingBankAccounts` マップ保存
  → INSERT 時に `w.bankAccount?.trim() || existingBankAccounts.get(w.cardName) || null` で引き継ぎ

### withdrawals/page.tsx — 残高警告機能追加
- `daysUntilJst(dateStr)`: JST（Asia/Tokyo）で引き落とし日まで何日かを計算
- `warnings` useMemo: 3段階警告（urgent/danger/caution）を balanceStatus.summary から生成
  - urgent: 3日以内 + 残高不足 → 🚨 赤点滅
  - danger: 7日以内 + 残高不足 → 🔴 赤
  - caution: 3日以内 + 残高OK → ⚠️ 黄
- ページトップ警告バナー（warnings.length > 0 のときのみ表示）
- 各行の引き落とし日セルに個別警告バッジ
- 口座残高セルに差額（+/-）の色付き表示（黒字=緑、赤字=赤）
- 口座未取得時のインフォバー（`accounts.length === 0` かつ `!loadingMapping`）
